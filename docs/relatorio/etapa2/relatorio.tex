...
rápida é a execução em Julia em relação ao Python.
 \begin{figure}[!htb]
    \centering
    \includegraphics[width=0.75\textwidth, height=0.420\textheight]{../../imagensEtapa2/boxplotCombinado.pdf}
    \caption{Comparação do tempo de execução entre Python e Julia para todos os casos}
 \end{figure}
\FloatBarrier 
Em comparação, podemos ver nas figuras 5 e 6 que o Julia consome uma proporção muito
maior de memória que o Python. Principalmente com um maior número de dimensões.
 \begin{figure}[!htb]
    \centering
...
e dados de resultados. Os primeiros tem uma linha extraída a cada segundo,
enquanto o segundo tem uma linha extraída a cada experimento. Cada linha possuí
5 variáveis preditoras e as restantes vem do experimento. Mantendo assim os dados
tidy; A criação dos scripts e códigos foi feita a base de pesquisa, tentativa e
...
Partindo desta etapa parcial o pretende-se: organizar os scripts auxiliares 
que estão e serão utilizados pelo experimento; Ampliar a análise ao incluir
a 3° dimensão que ainda não foi analisada e usando comparaçõess que forneçam
maior esclarecimento sobre os daods; Uma análise qualitativa dos resultados;
