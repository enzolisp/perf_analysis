...
dados <- dados %>%
  mutate(
    Mem_MiB = parse_mem_to_MiB(mem_raw),
    Mem_MB  = Mem_MiB * 1.048576,
    dim_type = as.numeric(str_remove(tolower(as.character(dim_raw)), "d")),
    L_value = as.numeric(L_value_raw)
  )
...
summary_mem <- dados %>%
  group_by(language, ContainerID, dim_type, L_value) %>%
...
summary_geral <- summary_mem %>%
  ...
  group_by(language, dim_type, L_value) %>%
  summarise(
...
ggsave("mem_bars_1d.png", plot_1d_bars, width = 10, height = 7)
ggsave("mem_bars_2d.png", plot_2d_bars, width = 10, height = 7)
ggsave("mem_bars_3d.png", plot_3d_bars, width = 10, height = 7)
