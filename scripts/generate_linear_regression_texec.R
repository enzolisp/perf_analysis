...
for (i in 1:nrow(combinations_2)) {
    ...
    p <- ggplot(dados_subset, aes(x = L_Value, y = t_exec)) +
        geom_jitter(width = if(dim == 1) 250 else if(dim == 2) 50 else 5, height = 0, alpha = 0.28, color = "black", size = 2) +
        geom_smooth(method = "lm", formula = y ~ x, color = color_fill, se = TRUE, fill = "gray80", linewidth = 1.5) +
        scale_x_continuous(breaks = unique_L_values) +
        labs(
            title = paste0("Linear regression: ", toupper(lang), " - ", toupper(dim), "D"),
            subtitle = "Execution time vs Problem Size (L)",
            x = "Problem Size (L)",
            y = "Time (s)"
        ) +
        theme_bw() + 
    ...
    ggsave(filename = sprintf("graphs/linear_regression_texec/linear_regression_texec_%s_%sd.pdf", lang, dim), plot = p, width = 6, height = 4)
